...
while win.events_struct.event_running:
        ...
        for entityType in list(WorldMap.grid[gridCell]):
            ...
            for entity in WorldMap.grid[gridCell][entityType]:
                ...
                # If entity is herbivore
                    ...
                    entity: Herbivore = entity
                    # The minimum distance to/and the apple that is closest to the entity
                    closestApple: Apple = None
                    minDistance: float = entity.dna.senserange.get_value() * HerbivoreFactors.SENSERANGE # Very big value
                    
                    # Amount of tiles to check for entity collision
                    nTiles = int(math.ceil(entity.dna.senserange.get_value() * HerbivoreFactors.SENSERANGE / WorldMap.cell_size))

                    # Herbivore position
                    position = entity.get_center_pos() - Vector(entity.dna.senserange.get_value() * HerbivoreFactors.SENSERANGE, entity.dna.senserange.get_value() * HerbivoreFactors.SENSERANGE)
                    ...
                    for tile in tiles:
                        ...
                        for apple in currentCell:
                            ...
                            if entity.collides(apple) and entity.energy < entity.max_energy:
                                key = WorldMap.key(apple)
                                WorldMap.remove_from_key(key, apple)
                                entity.energy += apple.glucose

                            elif distance < entity.dna.senserange.get_value() * 10 and distance < minDistance:
                                closestApple = apple
                    ...
                    if not win.events_struct.speedup:
                        ...
                        if win.events_struct.showdbg:
                            pygame.draw.circle(win.screen, tuple(DefinedColors.blue), tuple(entity.get_center_pos()), entity.dna.senserange.get_value() * 10, width=2)                        
                            draw_hud_text(win, str(entity.endposition), entity.get_center_pos())
                            draw_hud_text(win, str(entity.energy_need), entity.get_center_pos() + Vector(0, 20))
                            draw_hud_text(win, str(entity.max_energy), entity.get_center_pos() + Vector(0, 40))
                    cell = WorldMap.key(entity) # the key of the cell the entity is inside

                    ## Move the entity
                    if closestApple == None or entity.energy >= entity.max_energy:
    ...
    if not win.events_struct.speedup:
        ...
        for key, value in dna.get_dict().items():
            current_string = f"{key}: {round(value/2,3)}"
